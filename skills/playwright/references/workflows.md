...
For visual verification, show screenshots in the conversation instead of saving local files. If the user explicitly requests saved artifacts, run commands from `output/playwright/<label>/` to keep artifacts contained.
...
pwcli click e3
pwcli snapshot
# Show visual evidence in the conversation; save a screenshot only if requested.
```
